#> Called by arcane_core:internal/core/load

function arcane_core:internal/modules/mana/disable
function arcane_core:internal/modules/items/disable
function arcane_core:internal/modules/effects/disable
function arcane_core:internal/modules/cooldowns/disable
function arcane_core:internal/modules/animate/disable

function #arcane_core:hooks/modules/disable
